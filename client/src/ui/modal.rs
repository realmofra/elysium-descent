                                ...
                                tabs_parent
                                    .spawn((
                                        Node {
                                            ...
                                            border: if is_active {
                                                UiRect::bottom(Val::Px(3.0))
                                            } else {
                                                UiRect::all(Val::Px(0.0))
                                            }, // Scaled up by 1.5x from 2
                                            ..default()
                                        },
...
pub fn update_quest_list(
    ...
    let Some(quest_container_entity) = quest_container_query.iter().next() else {
        return;
    };
    ...
    for i in 0..10 {
        ...
        let quest_entity =
            spawn_quest_entry(&mut commands, &quest_objective, &font_assets, &ui_assets, i);
        commands
            .entity(quest_container_entity)
            .add_child(quest_entity);
    }
}
...
impl Plugin for ModalPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<ModalState>().add_systems(
            Update,
            (
